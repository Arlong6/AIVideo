...
【Pattern Interrupt 規則 — 對抗每 2 分鐘一次的流失波（retention 曲線的固定跳崖點）】
第 3、4 段（各 400-500 字 ≈ 2-2.5 分鐘）**段落中間**必須各插入 1 個節奏斷點，三選一：
  a) 直接對觀眾提問:「換作是你，你會相信他的說法嗎？」
  b) 急停短句 + 時間跳切:「三天後。電話響了.」
  c) 反差事實急轉:「而此時真正的兇手，正坐在警局對面的麵攤吃麵.」（必須是資料內的事實）
斷點必須融入敘事，不可寫成「接下來讓我們…」的主持人腔.
...
【5. 轉折（300-400字）】意外發展、新證據。段落中間插入 1 個 pattern interrupt
   （對觀眾提問 / 急停短句時間跳切 / 資料內的反差事實急轉），融入敘事不可出戲.
【6. 結局（300-400字）】破案/未破案、審判結果
【7. 反思（150-200字）】社會影響、法律改變
【8. 結語（80-100字）】留下餘韻、呼籲訂閱。訂閱呼籲必須給「具體理由」：
   說出頻道的系列承諾（例:「台灣懸案檔案，每週三集，下一集是○○案」的格式，
   系列名/案件必須真實存在於本頻道方向，不可捏造具體下集案名——用類型代替，
   例:「下一個，是一樁 30 年沒人敢碰的軍中懸案」）。禁止乾巴巴的「記得訂閱」.
