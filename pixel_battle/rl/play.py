...
def _get_vignette() -> pygame.Surface:
    ...
    if _VIGNETTE_CACHE is None:
        ...
        # vig is drawn straight onto the (possibly scaled) world surface, so
        # its pixel array must match vig's actual size, not the fight-coord
        # WIDTH/HEIGHT — the shim maps the full canvas to CANVAS_SCALED.
        vw, vh = vig.get_size()
